...
async def upgrade(db: BaseDBAsyncClient) -> str:
    return """
        INSERT INTO "docs" ("title", "slug", "urlpath", "public", "metadata", "markdown", "html", "order", "created_at", "updated_at", "parent_id", "updated_by_id")
        SELECT 'Home', '', '', TRUE, '{"subtitles": []}', '', '', 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, NULL, NULL
        WHERE NOT EXISTS (SELECT 1 FROM "docs" WHERE "urlpath" = '' AND "parent_id" IS NULL);"""


async def downgrade(db: BaseDBAsyncClient) -> str:
    return """
        DELETE FROM "docs" WHERE "urlpath" = '' AND "parent_id" IS NULL AND "slug" = '';"""
